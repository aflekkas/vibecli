...
For anything reachable through the agent loop: add or update a scenario in `templates/playground/scenarios/` (`{ input, expectContains }` JSON), wire the surface into `templates/playground/src/index.tsx` if not already there, run `bun run play:script scenarios/<file>.json`. The same scenarios run automatically in `bun run smoke` post-publish, so coverage compounds.
